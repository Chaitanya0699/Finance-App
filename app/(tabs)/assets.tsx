...
export default function AssetsScreen() {
  ...
  const totalPurchaseValue = mockAssets.reduce((sum, asset) => sum + asset.purchaseValue, 0);
  ...
  const renderSummaryCard = () => (
    <Animated.View style={[animatedCardStyle]}>
      <TouchableOpacity onPress={handleCardPress} activeOpacity={0.9}>
        <LinearGradient
          colors={totalGainLoss >= 0 ? ['#10B981', '#059669'] : ['#EF4444', '#DC2626']}
          start={{ x: 0, y: 0 }}
          end={{ x: 1, y: 1 }}
          ...
          <Text style={styles.summarySubtext}>
            {totalGainLoss >= 0 ? 'Total Appreciation' : 'Total Depreciation'}
          </Text>
        </LinearGradient>
      </TouchableOpacity>
    </Animated.View>
        ...
        {Object.entries(assetsByType).map(([type, value]) => (
          <View key={type} style={styles.breakdownItem}>
            <View style={styles.breakdownItemLeft}>
              <View style={[styles.breakdownDot, { backgroundColor: getTypeColor(type) }]} />
